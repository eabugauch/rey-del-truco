...
const MobileNavHeader = ({ onBack, title, subtitle, className = '' }) => {
  return (
    <div className={`mobile-nav-header ${className}`}>
      <div className="flex items-center justify-between p-4 pt-6 pb-6 safe-top">
        ...
        >
          ...
          <span className="back-text text-sm handwritten-normal">
            Volver
          </span>
        </button>
        ...
        <div className="flex-1 text-center px-4">
          <h1 className="text-2xl md:text-3xl handwritten-title ink-blue organic-text">
            {title}
          </h1>
          {subtitle && (
            <p className="text-xs handwritten-normal ink-blue opacity-70 mt-1">
              {subtitle}
            </p>
          )}
        </div>
